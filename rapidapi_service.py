...
def format_caption(raw: str, username: str = None, post_url: str = None) -> str:
    ...
    # متن اصلی به صورت نقل قول (دقیقاً همون چیزی که می‌خواستی)
    if text:
        lines.append("")
        lines.append(f"💬 <i>{text}</i>")   # نقل قول ایتالیک

    # هشتگ‌ها
    if hashtags:
        hashtag_line = " ".join(hashtags)
        lines.append("")
        lines.append(f"<code>{hashtag_line}</code>")
